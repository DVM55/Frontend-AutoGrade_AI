...
import { getQuizById, updateQuiz } from "../../service/quiz.service";
...
const IconBarChart = ({ size = 17, color = "#3d3a8c" }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const IconFileText = ({ size = 17, color = "#3d3a8c" }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const IconArrowLeft = ({ size = 18, color = "#6c757d" }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const mapApiToMeta = (quiz) => ({
  title: quiz.title || "",
  description: quiz.description || "",
  totalScore: String(quiz.totalScore ?? "10"),
  durationMinutes: quiz.durationMinutes ? String(quiz.durationMinutes) : "60",
  maxAttempts: quiz.maxAttempts ?? 1,
  startTime: toLocalDatetime(quiz.startTime),
  endTime: toLocalDatetime(quiz.endTime),
  allowReview: quiz.allowReview ?? false,
  quizStatus: quiz.quizStatus || "DRAFT",
  autoScore: quiz.autoScore ?? true,
  isRandom: quiz.isRandom ?? false,
  isPublic: (quiz.quizAccessType || "PUBLIC") === "PUBLIC",
  classId: (quiz.classes || []).map((c) => ({ id: c.id, title: c.title })),
  enableDuration: !!quiz.durationMinutes,
  enableTimeLimit: !!(quiz.startTime || quiz.endTime),
  quizCode: quiz.quizCode || null,
});

const mapApiToQuestions = (questions = []) =>
  questions.map((q) => ({
    id: q.id,
    content: q.content || "",
...
// Map API randomConfigs → local configs
const mapApiToConfigs = (randomConfigs = []) => {
  if (!randomConfigs || randomConfigs.length === 0)
    return [{ category: null, group: null, quantity: 1 }];
  return randomConfigs.map((cfg) => ({
    id: cfg.id,
    category: cfg.categoryQuestion
      ? { id: cfg.categoryQuestion.id, name: cfg.categoryQuestion.name }
      : cfg.categoryQuestionId
        ? { id: cfg.categoryQuestionId, name: cfg.categoryQuestionName || "" }
        : null,
    group: cfg.groupQuestion
      ? { id: cfg.groupQuestion.id, name: cfg.groupQuestion.name }
      : cfg.groupQuestionId
        ? { id: cfg.groupQuestionId, name: cfg.groupQuestionName || "" }
        : null,
...
const validateQuestion = (form) => {
  const errs = {};
  if (!form.content.trim() && !form.mediaObjectKey && !form.mediaUrl)
  ...
  if (form.questionType !== "SHORT_ANSWER") {
    if (form.options.length < 2) errs.options = "Cần ít nhất 2 đáp án.";
      ...
      if (texts.some((t, i) => texts.indexOf(t) !== i))
        errs.options = "Các đáp án không được trùng nhau.";
    }
  } else {
  ...
  return (
    <div
      ...
      style={{
        ...
        height: h,
        borderRadius: h / 2,
        background: checked ? "#3d3a8c" : "#d1d5db",
        position: "relative",
...
// ─── ClassSelector ────────────────────────────────────────────────────────────
const ClassSelector = ({ value, onChange, error, onClearError, disabled }) => {
  const [showModal, setShowModal] = useState(false);
  const removeClass = (id) => {
    if (!disabled) onChange(value.filter((c) => c.id !== id));
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      {!disabled && (
        ...
        >
          <IconPlus size={14} color={error ? "#dc2626" : "#3d3a8c"} /> Thêm lớp
        </button>
      )}
      {showModal && (
        <ClassSearch
...
const ConfigCard = ({
  ...
  >
    ...
    >
      ...
      <TogglePill checked={checked} onChange={onToggle} disabled={disabled} />
    </div>
    {checked && children && (
      <div
        style={{
    ...
    >
      ...
      >
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {/* Category */}
          <button
            onClick={() => !readOnly && setShowCategory(true)}
            disabled={readOnly}
            style={{
              flex: 1,
              minWidth: 140,
          ...
          {/* Group */}
          <button
            onClick={() => !readOnly && setShowGroup(true)}
            disabled={readOnly}
            style={{
              flex: 1,
              minWidth: 140,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <input
            ...
            disabled={readOnly}
            onChange={(e) =>
              !readOnly && onChange({ ...cfg, quantity: e.target.value })
            }
            style={{
              width: 76,
              padding: "6px 8px",
              ...
              background: error ? "#fff5f5" : readOnly ? "#f3f4f6" : "#fff",
              color: readOnly ? "#6c757d" : "#3d3a8c",
              outline: "none",
              fontFamily: "inherit",
...
const QuestionForm = React.forwardRef(
  (
    ...
    const set = (patch) => {
      if (!readOnly) onChange({ ...form, ...patch });
    };
      ...
      >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              ...
              {form.options.map((opt, idx) => {
                ...
                return (
                  ...
                  >
                    <button
                      onClick={() => !readOnly && toggleCorrect(idx)}
                      disabled={readOnly}
                      style={{
                        width: 26,
                        height: 26,
                    ...
                    <textarea
                      ...
                      style={taStyle({
                        ...
                        width: 0,
                        padding: "6px 10px",
                        border: `1.5px solid ${opt.isCorrect ? correctColors.border : "#e9ecef"}`,
                        background: opt.isCorrect ? correctColors.bg : "#fff",
              ...
              {!readOnly && form.options.length < 10 && (
                <button
                  onClick={addOption}
                  style={{
                    display: "inline-flex",
                ...
                >
                  ...
                  <textarea
                    ...
                    style={taStyle({
                      ...
                      width: 0,
                      padding: "6px 10px",
                      border: `1.5px solid ${correctColors.border}`,
                      background: correctColors.bg,
        ...
        <div
          style={{
            ...
            padding: "10px 16px",
            background: "#fafafa",
            borderTop: `1.5px solid ${readOnly ? "#e9ecef" : "#ede9fe"}`,
            flexWrap: "wrap",
            gap: 8,
          }}
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input
                ...
                disabled={autoScore || readOnly}
                onChange={(e) => {
                  if (!autoScore && !readOnly) {
                ...
                style={{
                  ...
                  textAlign: "center",
                  border: "1.5px solid #e9ecef",
                  background: autoScore || readOnly ? "#f3f4f6" : "#fff",
                  color: autoScore || readOnly ? "#6c757d" : "#3d3a8c",
                  outline: "none",
                  fontFamily: "inherit",
                  cursor: autoScore || readOnly ? "not-allowed" : "text",
                }}
              />
            </div>
          </div>
...
const QuizDetail = () => {
  ...
  const [configs, setConfigs] = useState([
    { category: null, group: null, quantity: 1 },
  ]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const quiz = res.data;
        setMeta(mapApiToMeta(quiz));
        setSavedStatus(quiz.quizStatus);
        setForms(mapApiToQuestions(quiz.questions || []));
        setConfigs(mapApiToConfigs(quiz.randomConfigs));
      } catch {
  ...
  const handleSubmit = async () => {
      ...
      if (meta.isRandom) {
        ...
        if (!meta.autoScore) {
          ...
          if (!hasScoreErrors) {
            ...
            if (
              Math.round(totalInput * 100) !==
              Math.round(Number(meta.totalScore) * 100)
            ) {
              setMetaErrors((p) => ({
                ...p,
                scoreSumError:
        ...
        if (Object.keys(qErrs).length > 0) {
          setAllErrors(qErrs);
          pendingScrollRef.current = Math.min(
            ...Object.keys(qErrs).map(Number),
          );
          setActiveTab("questions");
          return;
        }
    ...
    let payload;
    if (isPublished) {
      // Khi đã xuất bản: chỉ gửi meta, không gửi câu hỏi/cấu hình
      payload = base;
    } else if (meta.isRandom) {
      payload = {
        ...base,
  ...
  const allTabs = [
    ...
    ...(isPublished
      ? [
          { key: "stats", label: "Thống kê", Icon: IconBarChart },
          { key: "submissions", label: "Bài nộp", Icon: IconFileText },
        ]
      : []),
  ];
    ...
    <>
      <style>{`
        ...
        @media (max-width: 360px) {
          ...
          .ce-save-btn { padding: 8px 10px !important; }
        }
      `}</style>
      ...
      >
        ...
        >
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: hasErr ? "#dc2626" : active ? "#3d3a8c" : "#6c757d",
                    boxShadow: active
          ...
          >
            ...
            >
              ...
              <IconSave />
              <span className="ce-save-text">{loading ? "" : "Lưu"}</span>
            </button>
            <button
              onClick={handleBack}
              style={{
        ...
        <div style={{ flex: 1, padding: "20px 0" }}>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        style={{
                          ...inputSt("totalScore"),
                          cursor: isPublished ? "not-allowed" : undefined, // ← thêm
                        }}
                      />
                    </Field>
                    <Field
                    ...
                    <Field label="Trạng thái">
                      <div style={{ position: "relative" }}>
                        <select
                          ...
                          onChange={(e) => setM({ quizStatus: e.target.value })}
                          disabled={isPublished}
                          style={{
                            ...inputSt(null),
                            appearance: "none",
                  ...
                  >
                    {[
                      {
                        ...
                        val: meta.autoScore,
                        // Disabled nếu đã published HOẶC isRandom
                        disabled: isPublished || meta.isRandom,
                        onSet: (v) => {
                          if (isPublished || meta.isRandom) return;
                          setM({ autoScore: v });
                          setMetaErrors((p) => {
                            const n = { ...p };
                            delete n.scoreSumError;
                    ...
                    ].map(({ key, label, val, onSet, disabled }) => (
                      <div
                        key={key}
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          ...
                          height: 30,
                          borderRadius: 6,
                          border: `1.5px solid ${copied ? "#bbf7d0" : "#e9ecef"}`,
                          background: copied ? "#f0fdf4" : "#fff",
                          cursor: "pointer",
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          color: copied ? "#16a34a" : "#6c757d",
                          transition: "all .15s",
                        }}
                        onMouseEnter={(e) => {
                          if (copied) return;
              ...
              >
                ...
                >
                  ...
                  <Field label="Thời gian kết thúc">
                    <input
                      ref={
                        firstMetaErrKey === "endTime" ? metaErrorRef : undefined
                      }
                      className="exam-inp"
                      type="datetime-local"
                      value={meta.endTime}
          ...
          {activeTab === "questions" && (
            ...
            >
              {/* Toggle isRandom — chỉ hiển thị khi chưa published */}
              {!isPublished && (
                <div
                  onClick={() => {
          ...
          {/* ── TAB: Thống kê ── */}
          {activeTab === "stats" && isPublished && <StatsTab quizId={quizId} />}

          {/* ── TAB: Bài nộp ── */}
          {activeTab === "submissions" && isPublished && (
            <SubmissionsTab quizId={quizId} />
          )}
        </div>
      </div>
    </>
