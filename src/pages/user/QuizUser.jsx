...
import { getQuizByClassId } from "../../service/quiz.service";

const QuizUser = ({ classId }) => {
  ...
  const fetchQuizzes = async (page = 0) => {
    try {
      setLoading(true);
      const res = await getQuizByClassId(classId, page, size);
        ...
        <div className="qz-list">
          {quizzes.map((quiz) => (
            <div
              key={quiz.id}
              className="qz-item"
              onClick={() => navigate(`/user/quizzes/${quiz.id}`)}
            >
              ...
              <div className="qz-item__info">
                <div className="qz-item__status">
                  {quiz.submitted ? (
                    <span className="qz-badge qz-badge--submitted">
                      <i className="bi bi-check-circle-fill" /> Đã nộp
                    </span>
                  ) : (
                    <span className="qz-badge qz-badge--pending">
                      <i className="bi bi-clock" /> Chưa nộp
                    </span>
                  )}
                </div>
                <div className="qz-item__title">{quiz.quizTitle}</div>
                <div className="qz-item__desc">
                  {quiz.quizDescription || "Không có mô tả"}
                </div>
              </div>
