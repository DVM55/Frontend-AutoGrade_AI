...
const CategorySearch = ({ value, onConfirm, onClose }) => {
  ...
  const fetchCategories = async (name = "", pageNum = 0, isAppend = false) => {
    if (pageNum === 0) setCategoryLoading(true);
    ...
    try {
      ...
      setCategories((prev) => (isAppend ? [...prev, ...newItems] : newItems));
  ...
  const handleSearch = (e) => {
    ...
    timerRef.current = setTimeout(() => fetchCategories(val, 0, false), 200);
  };
  ...
  const handleScroll = useCallback(() => {
    ...
    if (nearBottom) {
      ...
      setPage(nextPage);
      fetchCategories(categorySearch, nextPage, true);
    }
  }, [loadingMore, hasMore, page, categorySearch]);
      ...
      >
        ...
        >
          {categoryLoading ? (
            ...
            <>
              {categories.map((cat) => {
                const isSelected = selectedTemp?.id === cat.id;
                return (
                  <div
                    key={cat.id}
                    onClick={() => setSelectedTemp(cat)}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: "10px",
