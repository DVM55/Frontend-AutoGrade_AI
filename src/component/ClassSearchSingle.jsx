...
const ClassSearchSingle = ({ value, onConfirm, onClose, title }) => {
  ...
  const fetchClasses = async (name = "", pageNum = 0, isAppend = false) => {
    ...
    try {
      ...
      setClasses((prev) => (isAppend ? [...prev, ...newItems] : newItems));
  ...
  const handleScroll = useCallback(() => {
    ...
    if (nearBottom) {
      ...
      setPage(nextPage);
      fetchClasses(search, nextPage, true);
    }
      ...
      >
        ...
        >
          {loading ? (
            ...
            <>
              {classes.map((cls) => {
                const isSelected = selectedTemp?.id === cls.id;
                return (
                  <div
                    key={cls.id}
                    onClick={() => setSelectedTemp(cls)}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: "10px",
...
export default ClassSearchSingle;
